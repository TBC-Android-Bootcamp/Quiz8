...
class NewsActivity : AppCompatActivity() {
    ...
    private fun init() {
        recyclerViewAdapter = NewsRecyclerViewAdapter(newsRecyclerView, news)
            ...
            if (news.size != 0) {
                if (!news[news.size - 1].isLast) {
                    newsRecyclerView.post {
                        ...
                        news.add(newsModel)
                        recyclerViewAdapter.notifyItemInserted(news.size - 1)
            ...
            object : FutureCallBack<String> {
                override fun done(result: String) {
                    ...
                    if (lastPosition > 0) {
                        ...
                        recyclerViewAdapter.notifyItemRemoved(news.size)
                    }
                    recyclerViewAdapter.setLoaded()
                    news.addAll(Gson().fromJson(result, Array<NewsModel>::class.java).toList())
                    if (news.isNotEmpty() && news.size != 1) recyclerViewAdapter.notifyItemMoved(
                        lastPosition,
                        news.size - 1
                    )
